...
const VooidShop = () => {
  ...
  const applyFilters = useCallback((filters) => {
    ...
    const filtered = productsData.filter((product) => {
      const availabilityMatch = !availability.length || availability.includes(product.availability);
      const priceMatch = (!priceRange.min || product.price >= priceRange.min) &&
        (!priceRange.max || product.price <= priceRange.max);
      const colorMatch = !colors.length || colors.includes(product.color);
    ...
    updateState({ filteredProducts: filtered });
  }, [updateState]);
  ...
  const handleSortChange = useCallback((sortOption) => {
    ...
    sortedProducts.sort(sortMap[sortOption]);
    updateState({ filteredProducts: sortedProducts });
  }, [filteredProducts, updateState]);
  ...
  const handleAddToCart = useCallback((product) => {
    ...
    setTimeout(() => updateState({ cartEntering: true }), 200);
  ...
  }, [addToCartContext, updateState]);

  const handleBuyNow = useCallback(() => {
    console.log("Redirigiendo a checkout con:", lastAddedProduct);
    updateState({ showCartNotification: false, cartEntering: false });
  }, [lastAddedProduct, updateState]);
  ...
  const toggleMobileFilters = useCallback(() => {
    updateState(prev => ({ showMobileFilters: !prev.showMobileFilters }));
  }, [updateState]);

  return (
    <section className="relative min-h-screen">
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-br from-[#1a0f1a] via-[#2e1c2b] to-[#110911]" />
      ...
      <div className="relative z-10 md:[zoom:0.8] 2xl:[zoom:1]">
        <div className="relative mx-auto max-w-screen-2xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8 w-[90%]">
          <header className="mb-12">
            ...
            <div className="w-20 h-0.5 bg-white/40 mb-6" />
          ...
          <div className="mt-8 lg:mt-12 lg:grid lg:grid-cols-4 lg:items-start lg:gap-8">
            ...
            <div className="hidden lg:block space-y-6">
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
                <FilterSection onFilterChange={applyFilters} filteredProducts={filteredProducts.length} allProducts={productsData} />
              </motion.div>
            </div>
            ...
            <div className="lg:col-span-3">
                ...
                <div className="bg-gradient-to-r from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-xl p-2 shadow-2xl shadow-black/30">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <button onClick={toggleMobileFilters} className="lg:hidden flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-500/30 text-purple-200 hover:border-purple-500/50 hover:from-purple-500/30 hover:to-pink-500/30 transition-all duration-300">
                      ...
                      <div className="hidden sm:block pl-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-white"><span className="font-bold">{filteredProducts.length}</span> productos encontrados</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2 bg-white/5 backdrop-blur-sm border border-white/20 rounded-lg p-1">
                        <motion.button onClick={() => handleViewModeChange('grid')} className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all duration-200 ${viewMode === 'grid' ? 'bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-white border border-purple-500/30' : 'text-white/60 hover:text-white hover:bg-white/10'}`} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                          <Grid3x3 className="w-5 h-5" />
                          <span className="text-sm font-medium hidden sm:inline">Grilla</span>
                        </motion.button>
                        <div className="w-px h-6 bg-white/20" />
                        <motion.button onClick={() => handleViewModeChange('list')} className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all duration-200 ${viewMode === 'list' ? 'bg-gradient-to-r from-teal-500/20 to-emerald-500/20 text-white border border-teal-500/30' : 'text-white/60 hover:text-white hover:bg-white/10'}`} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  ...
                  <div className="sm:hidden mt-4 pt-4 border-t border-white/10">
                    <div className="flex items-center justify-between">
                      <div className="text-sm text-white"><span className="font-bold">{filteredProducts.length}</span> productos</div>
                      <div className="text-xs text-white/40">{viewMode === 'grid' ? 'Grilla' : 'Lista'}</div>
                    </div>
                  </div>
                </div>
              </motion.div>
              ...
              {/* Filtros móviles */}
              {showMobileFilters && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} transition={{ duration: 0.3 }} className="lg:hidden mb-6">
                  <div className="bg-gradient-to-r from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={toggleMobileFilters} className="text-white/60 hover:text-white"><X className="w-5 h-5" /></button>
                    </div>
                    <div className="mb-6">
              ...
              <div className="mt-4">
                {viewMode === 'grid' ? (
                  <ProductGrid products={filteredProducts} onProductClick={handleProductClick} formatPrice={formatPrice} />
                ) : (
                  <ProductList products={filteredProducts} onProductClick={handleProductClick} formatPrice={formatPrice} />
                )}
              </div>
            </div>
          </div>
      ...
      {isModalOpen && (
        <ModalContainer onClose={handleCloseModal}>
          <ProductModal product={selectedProduct} onClose={handleCloseModal} onAddToCart={handleAddToCart} isExiting={modalExiting} />
        </ModalContainer>
      )}

      {showCartNotification && (
        <CartNotification product={lastAddedProduct} onBuyNow={handleBuyNow} onContinueShopping={handleContinueShopping} onClose={handleCloseCartNotification} formatPrice={formatPrice} cartItemsCount={cartItemsCount} />
      )}

      <style jsx>{`
        @keyframes float {
          0% { transform: translate(0, 0) rotate(0deg); opacity: 0.1; }
          50% { transform: translate(${Math.random() * 100 - 50}px, ${Math.random() * 100 - 50}px) rotate(180deg); opacity: 0.3; }
